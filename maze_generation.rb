...
def psol
  ...
  $lp.matrix_solution["x"].each_index do |i|
    ...
    row.each_index do |j|
      ...
      row.each_index do |v|
        row.each_index do |w|
          i_v = $lp.matrix_solution["p"][i][j][v][w] rescue 0
          if i_v == 1.0
...
constraints << "sum(i in (0..#{n-1}), v in (1..#{n-2}), w in (1..#{n-2}), p[i][#{n-1}][v][w]) = 0"
...
# todo: turn this into a single sum constraint, which should remove a lot of potential solution space
constraints << "forall(i in (1..#{n-2}), j in (1..#{n-2}), sum(v in (1..#{n-2}), w in (1..#{n-2}), p[i][j][v][w]) + x[i][j] = 1)"
# constraints << "sum(i in (0..#{n-1}), j in (0..#{n-1}), x[i][j]) + sum(i in (0..#{n-1}), j in (0..#{n-1}), v in (0..#{n-1}), w in (0..#{n-1}), p[i][j][v][w]) = #{n*n}"
...
# if there is a square on incorrect path originating at v,w then there is a square adjacent to v,w also on that incorrect path
...
constraints << "sum(i in (0..#{n-1}), j in (0..#{n-1}), x[i][j]) <= #{n*n -2*n + 3}"
constraints << "sum(i in (1..#{n-2}), j in (1..#{n-2}), v in (1..#{n-2}), w in (1..#{n-2}), p[i][j][v][w]) <= #{6*n - 21}"
# constraints << "sum(i in (1..#{n-2}), j in (1..#{n-2}), v in (1..#{n-2}), w in (1..#{n-2}), p[i][j][v][w]) >= #{n+2}"
